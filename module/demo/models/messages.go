...
type Message struct {
	Id        int64  `json:"id, omitempty" gorm:"column(id);pk;unique;auto_increment"`
	UserId    int64  `json:"user_id" gorm:"column(user_id);size(11)"`
	ToUserId  int64  `json:"to_user_id" gorm:"column(to_user_id);size(11)"`
	Type      int    `json:"type, omitempty" gorm:"column(type);size(4)"`
	SubType   int    `json:"sub_type, omitempty" gorm:"column(sub_type);size(4)"`
	Title     string `json:"title, omitempty" gorm:"column(title);varbinary"`
	Url       string `json:"url, omitempty" gorm:"column(url);varbinary"`
	Viewed    int    `json:"viewed" gorm:"column(viewed);size(1)"` // 1: viewed, 0:not-viewed
